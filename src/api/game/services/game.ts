...
async function createManyToManyData(products) {
  ...
  products.forEach((product) => {
    ...
    const createCall = (set, entityName) =>
      Array.from(set).map(async (name) => await create(name, entityName));
...
async function setImage({ image, game, field = "cover" }) {
  ...
  try {
    ...
    await axios({
      method: "POST",
      //para funcionar o upload via service tem que habilitar o upload no Stripe
          ...
          data: {
            ...
            categories: await Promise.all(
              product.genres.map(({ name }) =>
                getByName(name, CATEGORIES_SERVICE)
              )
            ),
...
export default factories.createCoreService("api::game.game", () => ({
  async populate(params) {
    try {
      ...
      const gogApiUrl = `
    https://catalog.gog.com/v1/catalog?${QueryString.stringify(params)}`;
